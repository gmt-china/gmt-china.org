#!/bin/bash
#本脚本修改自鄭懷傑的等高線地圖及地形剖面指令稿(http://gmt-tutorials.org/contour_and_profile.html#id12)
PS=maunakea.ps
...
gmt project -C-155.65/19.75 -E-155.28/19.9 -G0.1 -Q | gmt grdtrack -Gmaunakea.grd > gmt.xy
    # 给定测线的起点(-C)和终点(-E)的经纬度，以及步长(-G)，project 输出经纬度坐标通过管道传输给 grdtrack，grdtrack 从地形文件中取样，把获得的信息输出到 gmt.xy 文件，gmt.xy 文件四列，依次是经度、纬度、距离和高程
gmt psxy gmt.xy -R -J -Wthick,darkred -O -K >> $PS
    # psxy 使用 gmt.xy 的前两列画测线
gmt pstext -R -J -F+j+f14p,25,darkred -O -K >> $PS << TEXTEND
-155.65 19.75 LT A
-155.28 19.9 LT B
TEXTEND
    # 测线的起点和终点标记为 A 和 B
...
gmt psbasemap -R -J -BWS -Bxa5f1+l"Distance of A-B profile (km)" -Bya1000+l"Height (m)" -O -K >> $PS
    # 先画 NE 两边的外框，再画 WS 两边的外框
gmt psxy -R -J -O -T >> $PS
gmt psconvert -A -Tg -E1000 $PS
rm -rf gmt.*
