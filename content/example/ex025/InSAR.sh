...
#自定义一个模板色标文件,HSV值，具体含义，详见：
#https://docs.gmt-china.org/latest/basis/color/?highlight=hsv#hsv
...
#在左下角的色标后面插入一个白色背景框
gmt psbasemap -R -J$J -K -O -DjBL+w2c/4c -F+gwhite >>$PS
# 绘制色标尺
gmt psscale -R$R -J$J  -DjBL+w3c/0.5c+o0.3c/0.3c  -Cmy.cpt -Bx20 -By+l'unit' -K -O >>$PS

#结束绘图并转换为JPG图片 
gmt psxy -R$R -J$J -O -T>>$PS
gmt psconvert $PS -A -Tg -E
