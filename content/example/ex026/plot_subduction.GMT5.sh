...
EOF
# 绘制剖面图
gmt psbasemap -R0/15/0/700 -JX10c/-4c -Bya200f100+l"Focal depth (km)" -Bxa2f1+l"Distance"+u"\260" -BWS -X14c -Y2c -K -O >> $PS
gmt psbasemap -R -J -B0 -Bne -K -O >> $PS
# 在剖面图上绘制震源球,剖面宽度为400
gmt pscoupe $FM -R -J -Q -L -Sm0.3c -Aa126/42/146/40/90/400/0/700f -Zdepth.cpt -K -O >> $PS
# 绘制文字标注
echo 5 400 Benioff zone | gmt pstext -R -J -F+f12p,10,blue=~1p,gray+a30+jBL -K -O >> $PS
# 绘制图例
gmt pslegend -R -J -DjBR+w1i+l1.5+o0.05i/0.04i -F+g255+p0.25p -K -O >> $PS << EOF
H 8 4 Events (Mw>4.5)
...
EOF
# 沿测线AB绘制地形高度
gmt psbasemap -R0/15/-4000/6000 -JX10c/1c -Bnwse -Y4c -K -O >> $PS
# 标注AB位置
gmt pstext -R -J -F+f8p+jBC -N -K -O >> $PS << EOF
0 6000 A
15 6000 B
EOF
# 沿测线提取地形高度
...
# 将海平面以下填充为颜色
gmt psxy -R -J -Wblack -Gblue -L+y-4000 -K -O >> $PS << EOF
0 0
15 0
EOF
# 将地形填充为灰色
